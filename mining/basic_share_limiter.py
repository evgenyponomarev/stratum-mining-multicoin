...
class BasicShareLimiter(object):
    ...
    def update_litecoin_difficulty(self, wallet):
        ...
        if 'timestamp' not in self.litecoin or self.litecoin['timestamp'] < int(
            ...
            self.litecoin['difficulty'] = (yield Interfaces.template_registries[wallet].bitcoin_rpc.getdifficulty())
    ...
    def submit(self, wallet, connection_ref, job_id, current_difficulty, timestamp, worker_name):
        ...
        if (avg > self.tmax and current_difficulty > settings.VDIFF_MIN_TARGET):
            ...
            if settings.VDIFF_X2_TYPE:
                ...
                if settings.USE_COINDAEMON_DIFF:
                    self.update_litecoin_difficulty(wallet)
        ...
        connection_ref().rpc('mining.notify',
                             [work_id, prevhash, coinb1, coinb2, merkle_branch, version, nbits, ntime, False, ],
