...
class DB_Sqlite():
    ...
    def updateStats(self, averageOverTime):
        ...
        self.dbc.execute("select username,SUM(difficulty) from shares where time > :time group by username",
                         {'time': stime})
        total_speed = 0
        sqldata = []
    ...
    def archive_found(self, found_time):
        self.dbc.execute(
            "insert into shares_archive_found select * from shares where upstream_result = 1 and time <= :time",
            {'time': found_time})
        self.dbh.commit()
    ...
    def import_shares(self, data):
        ...
        for k, v in enumerate(data):
            if settings.DATABASE_EXTEND:
                ...
                sqldata.append(
                    {'time': v[4], 'rem_host': v[6], 'username': v[0], 'our_result': v[5], 'upstream_result': 0,
                     'reason': v[9], 'solution': '',
        ...
        if settings.DATABASE_EXTEND:
            self.dbc.executemany("insert into shares " + \
                                 ...
                                 "VALUES (:time,:rem_host,:username,:our_result,:upstream_result,:reason,:solution,:block_num,:prev_block_hash,:ua,:diff)",
                                 sqldata)
            ...
            if best_diff > round_best_share:
                self.dbc.execute("update pool set value = :val where parameter = 'round_best_share'",
                                 {'val': best_diff})
            ...
            for k, v in checkin_times.items():
                sqldata.append(
                    {'last_checkin': v["time"], 'addshares': v["shares"], 'addrejects': v["rejects"], 'user': k})
            self.dbc.executemany(
                "update pool_worker set last_checkin = :last_checkin, total_shares = total_shares + :addshares, " + \
    ...
    def found_block(self, data):
        ...
        self.dbc.execute(
            "update shares set upstream_result = :usr, solution = :sol where time = :time and username = :user",
            {'usr': data[5], 'sol': data[2], 'time': data[4], 'user': data[0]})
        if settings.DATABASE_EXTEND and data[5] == True:
            self.dbc.execute("update pool_worker set total_found = total_found + 1 where username = :user",
                             {'user': data[0]})
            self.dbc.execute("select value from pool where parameter = 'pool_total_found'")
            total_found = int(self.dbc.fetchone()[0]) + 1
            self.dbc.executemany("update pool set value = :val where parameter = :parm",
                                 [{'val': 0, 'parm': 'round_shares'},
    ...
    def insert_user(self, username, password):
        ...
        self.dbc.execute("insert into pool_worker (username,password) VALUES (:user,:pass)",
    ...
    def check_password(self, username, password):
        ...
        self.dbc.execute("select COUNT(*) from pool_worker where username = :user and password = :pass",
                         {'user': username, 'pass': password})
        data = self.dbc.fetchone()
        if data[0] > 0:
            return True
    ...
    def update_worker_diff(self, username, diff):
        self.dbc.execute("update pool_worker set difficulty = :diff where username = :user",
                         {'diff': diff, 'user': username})
        self.dbh.commit()
    ...
    def update_pool_info(self, pi):
        self.dbc.executemany("update pool set value = :val where parameter = :parm",
                             [{'val': pi['blocks'], 'parm': "bitcoin_blocks"},
    ...
    def get_workers_stats(self):
        self.dbc.execute(
            "select username,speed,last_checkin,total_shares,total_rejects,total_found,alive,difficulty from pool_worker")
        ret = {}
        for data in self.dbc.fetchall():
    ...
    def update_version_5(self):
        ...
        self.dbc.execute(
            "CREATE TABLE pool_worker(username TEXT, password TEXT, speed INTEGER, last_checkin DATETIME, total_shares INTEGER default 0, total_rejects INTEGER default 0, total_found INTEGER default 0, alive INTEGER default 0, difficulty INTEGER default 0, primary key(username))")
